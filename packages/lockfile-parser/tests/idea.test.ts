...
test.each(pnpmV9LockfileUrls)(
  "pnpm dependency specifier should only have known patterns",
  async (url) => {
    ...
    expect(unknownSpecifiers).toHaveLength(0);
  }
);

afterAll(async () => {
  await Bun.write(
    "all-unknown-specifiers.json",
    JSON.stringify(allUnknownSpecifiers, null, 2)
  );
});
