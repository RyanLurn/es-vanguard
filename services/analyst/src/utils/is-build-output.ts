import type {
  SkippedBuildOutputContentFile,
  SkippedBuildOutputPathFile,
...
export const buildOutputContentSkipReasons = [
  "webpack signature",
 ...
 */
export function isBuildOutputContent({
  content,
  filePath,
}: {
  content: string;
  filePath: string;
}): SkippedBuildOutputContentFile | undefined {
  // 1. Webpack / Bundler Signatures
  if (content.includes("__webpack_require__")) {
    const report: SkippedBuildOutputContentFile = {
      path: filePath,
      category: "build_output_content",
      reason: "webpack signature",
    };
    return report;
  }
